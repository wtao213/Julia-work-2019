...
###
df = DataFrame(CSV.File("C://Users//012790//Desktop//value_segmentation_2021_11//cus_20211115.csv", missingstring = ["", "NULL"]))
...
#############################
CSV.write("C://Users//012790//Desktop//value_segmentation_2021_11//cus_full_20211115.csv", df3)
...
df = DataFrame(CSV.File("C://Users//012790//Desktop//value_segmentation_2021_11//cus_full_20211115.csv", missingstring = ["", "NULL"]))
...
Equity_t0_qvals = round.(quantile(collect(skipmissing(df[!, :Equity_t0])), pcuts))        # find data values
...
# check the transfered value
pfine = 0.0:0.01:1.0
vfine = round.(quantile(df[!, :Equity_t0], pfine))

vip = LinRange(vfine[begin], vfine[end], 100)
pip = wblip(vip)

plot(vfine, pfine, label = "Empirical CDF")
plot!(vip, pip, label = "Interpolation CDF", legend = :bottomright, xformatter = x -> string("\$", Int(x / 1e3), "K"))
...
pcuts = 0.0:0.05:1.0                       # desired percentiles
trade_t_3_qvals = round.(quantile(collect(skipmissing(df[!, :trade_t_3])), pcuts))        # find data values
...
pcuts = 0.0:0.05:1.0                       # desired percentiles
AssetsTotal_bal_t_3_qvals = round.(quantile(collect(skipmissing(df[!, :AssetsTotal_bal_t_3])), pcuts))        # find data values
...
pcuts = 0.0:0.05:1.0                       # desired percentiles
WSINVEST_potential_qvals = round.(quantile(collect(skipmissing(df[!, :WSINVEST_potential])), pcuts))        # find data values
...
length(df[df.Income.>300000, :Income])
length(df[df.Income.>120000, :Income])
length(df[df.Income.>=100000, :Income])
...
df[df.age_t0.>20, [:rank, :age_group_quantile, :age_t0, :Income]]
...
df[df.age_t0.==21, [:D3_ttl_score_lip, :rank, :age_group_quantile, :age_t0, :Income]]
...
d1_med = median(df[!, :D1_ttl_score_lip])
d2_med = median(df[!, :D2_score_lip])
d3_med = median(df[!, :D3_ttl_score_lip])
len = length(df[!, 1])
...
CSV.write("C://Users//012790//Desktop//value_segmentation_2021_11//cus_profile_lip_2by2_EA_v2.csv", ai_df)
CSV.write("C://Users//012790//Desktop//value_segmentation_2021_11//cus_seg_final_604k_v2.csv", DF)



###########################################
# scoring criteria need to stored as Well
[pcuts Equity_t0_qvals trade_t_3_qvals AssetsTotal_bal_t_3_qvals WSINVEST_potential_qvals age_group_quantile_qvals]

scoring_file = DataFrame(pcuts = pcuts, Equity_t0_qvals = Equity_t0_qvals, trade_t_3_qvals = trade_t_3_qvals,
      AssetsTotal_bal_t_3_qvals = AssetsTotal_bal_t_3_qvals, WSINVEST_potential_qvals = WSINVEST_potential_qvals,
      age_group_quantile_qvals = age_group_quantile_qvals,
      d1_med = d1_med,
      d2_med = d2_med,
      d3_med = d3_med
)
...
##
CSV.write("C://Users//012790//Desktop//value_segmentation_2021_11//scoring_file_20211117.csv", scoring_file)
